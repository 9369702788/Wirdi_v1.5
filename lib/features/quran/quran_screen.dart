...
class _SurahReaderScreenState extends State<SurahReaderScreen> {
  ...
  void _setupPlayer() {
    _player.onPlayerComplete.listen((_) {
      if (_repeatCurrent && _playingAyah != null) {
        _playAyah(_playingAyah!, keepRepeat: true);
      } else {
        setState(() {
          _playingAyah = null;
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تعذر تحميل التفسير — تحقق من الاتصال')),
          );
        }
  ...
  Future<void> _playWholeSurah() async {
    ...
    try {
      await _player.play(UrlSource(AppSources.surahAudioUrl(widget.surah.number, reciter: _reciter)));
    } catch (e, st) {
      AppLogger.error('Surah audio playback failed', error: e, stackTrace: st);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('تعذر تشغيل الصوت: $e')),
        );
        setState(() => _playingWholeSurah = false);
      }
    } finally {
      if (mounted) setState(() => _isBuffering = false);
    }
