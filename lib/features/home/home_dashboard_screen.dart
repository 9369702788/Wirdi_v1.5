...
class _WeekSummaryCard extends StatelessWidget {
  ...
  static const _dayLetters = ['أحد', 'إثن', 'ثلا', 'أرب', 'خمي', 'جمع', 'سبت'];

  @override
  Widget build(BuildContext context) {
    final activeDays = summary.where((d) => d.hasAnyActivity).length;
    final targetMetDays = summary.where((d) => d.wirdTargetMet).length;
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                const Text('آخر 7 أيام', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                Text('$activeDays من 7 أيام نشطة', style: const TextStyle(color: AppColors.mutedText, fontSize: 12)),
              ],
            ),
            ...
            Row(
              ...
              children: summary.map((day) {
                final weekday = day.date.weekday % 7; // 0=Sunday to match _dayLetters
                final intensity = day.wirdTargetMet
                    ? 1.0
                    : (day.hasAnyActivity ? 0.5 : 0.12);

                return Column(
                  children: [
                    Semantics(
                      label: '${_dayLetters[weekday]}: ${day.wirdPages} صفحة، ${day.azkarCompleted} ذكر، ${day.tasbeehTotal} تسبيحة، ${day.prayersDone} صلوات',
                      child: Container(
                        width: 30,
                        height: 30,
                        decoration: BoxDecoration(
                        ...
                        alignment: Alignment.center,
                        child: day.wirdTargetMet
                            ? const Icon(Icons.check, size: 16, color: Colors.white)
                            : null,
                      ),
                    ),
                    const SizedBox(height: 4),
                    Text(_dayLetters[weekday], style: const TextStyle(fontSize: 10, color: AppColors.mutedText)),
                  ],
                );
              }).toList(),
            ),
            const SizedBox(height: 10),
            Text(
              'أتممت هدف الورد اليومي في $targetMetDays من آخر 7 أيام',
              style: const TextStyle(color: AppColors.mutedText, fontSize: 12),
            ),
          ],
        ),
