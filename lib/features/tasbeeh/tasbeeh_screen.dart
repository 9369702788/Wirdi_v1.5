...
class _TasbeehPhrase {
  ...
  final int target;
  final bool isCustom;
  const _TasbeehPhrase(this.id, this.text, this.target, {this.isCustom = false});
}

const _builtInPhrases = [
        ...
        content: Column(
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'نص الذكر', hintText: 'مثال: لا حول ولا قوة إلا بالله'),
            ),
        ...
        children: [
          ...
          Expanded(
            child: Center(
              child: Semantics(
                button: true,
                label: 'عداد تسبيح ${_selected.text}، العدد الحالي $_today من ${_selected.target}',
          ...
          const Padding(
            ...
            child: Text('اضغط للتسبيح — اضغط مطولًا على سبحة مخصصة لحذفها', style: TextStyle(color: AppColors.mutedText, fontSize: 12)),
          ),
        ],
      ),
    );
